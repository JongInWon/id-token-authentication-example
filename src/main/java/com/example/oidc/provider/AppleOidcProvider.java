...
public class AppleOidcProvider implements OidcProvider {
	...
	public AppleOidcProvider(
		@Value("${social.apple.jwksUrl}") final String jwksUrl,
		@Value("${social.apple.issuer}") final String issuer,
		@Value("${social.apple.audience}") final String audience
	) {
		this.issuer = issuer;
		this.audience = audience;

		try {
			this.appleJwkProvider = new JwkProviderBuilder(new URL(jwksUrl))
				.cached(10, 7, TimeUnit.DAYS) // 최대 10개 키를 7일간 캐싱
				.build();
		} catch (MalformedURLException e) {
			throw new RuntimeException("유효하지 않은 Apple JWKS URL입니다.", e);
		}
	}
	...
	private DecodedJWT decodeToken(final String idToken) {
		...
		try {
			...
			throw new RuntimeException("Apple id token 디코딩에 실패했습니다.", e);
		}
	}
	...
	private Jwk fetchJwk(final String kid) {
		...
		try {
			return appleJwkProvider.get(kid);
		} catch (JwkException e) {
			throw new RuntimeException("Apple JWKS 처리에 실패했습니다.", e);
		}
	}
